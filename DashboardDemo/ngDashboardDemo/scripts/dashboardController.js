...
(function (DashboardDemo) {
    var DashboardController = (function () {
        function DashboardController(dashboardService, $location, $mdDialog, $cookies, $route, $routeParams, $templateCache) {
            var _this = this;
            ...
            this.invData = [];
            this.invLabels = ['0-3 Months', '4-6 Months', '7-9 Months', '10-12 Months', '13-24 Months', '25+ Months'];
            this.$cookies.put('useraccess', "6152617337");
            //Get cookie value of current user
            var storesString = this.$cookies.get('useraccess');
            var storesStringArray = storesString.split(",");
        ...
        DashboardController.prototype.displayPartsInv = function (inventory) {
            if (inventory === void 0) { inventory = []; }
            var zeroToThree = 0;
            var fourtoSix = 0;
            var sevenToNine = 0;
            var tenToTwlve = 0;
            var thrteenToTwntyfr = 0;
            var twntyFiveGrtr = 0;
            var totalInvBalance = 0;
            this.invData = [];
        ...
        //Dashboard Modals
        //
        DashboardController.prototype.chartConfig = function (ChartJsProvider) {
            ChartJsProvider.defaults.global.responsive = true;
        };
        DashboardController.prototype.invDialog = function (inv) {
            if (inv === void 0) { inv = []; }
            this.$mdDialog.show({
        ...
        DashboardController.prototype.saveInCookies = function () {
        ...
        function AcctCompsController($mdDialog, year, dashboardService) {
            ...
            this.dashboardService.listAcctAll().$promise.then(function (results) {
                ...
                for (var i = 0; i < _this.months.length; i++) {
                    tempA.push(_this.months[i].net);
                    tempB.push(_this.oldMonths[i].net);
    ...
    var InvModalController = (function () {
        function InvModalController($mdDialog, inv) {
            if (inv === void 0) { inv = []; }
            this.$mdDialog = $mdDialog;
            this.inv = inv;
        }
        InvModalController.prototype.closeDialog = function () {
            this.$mdDialog.hide();
        };
        return InvModalController;
    })();
    DashboardDemo.InvModalController = InvModalController;
